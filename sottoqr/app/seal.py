...
def generate_certificate(image_path: str, filename: str, integrity_score: float,
    ...
    if os.path.exists(fist_path):
        try:
            ...
            fist.thumbnail((380, 380))
            faded = Image.new("RGBA", fist.size, (0, 0, 0, 0))
            r, g, b, a = fist.split()
            a = a.point(lambda p: int(p * 0.10))
            faded = Image.merge("RGBA", (r, g, b, a))
            card.paste(faded, (CARD_W - fist.width - 40, CARD_H - fist.height - 20), faded)
        except Exception:
            pass
    ...
    draw.rectangle([0, 0, CARD_W, 8], fill=RED)
    draw.rectangle([0, 8, CARD_W, 76], fill=BLACK)
    draw.text((26, 24), "SOTTOQR", font=_load_font(28), fill=YELLOW)
    draw.text((26, 54), "Truth Verification Seal", font=_load_font(14), fill=MUTED)

    # thumbnail with a thin red frame
    try:
        thumb = Image.open(image_path).convert("RGB")
        thumb.thumbnail((300, 300))
        frame_box = [24, 104, 24 + thumb.width + 6, 104 + thumb.height + 6]
        draw.rectangle(frame_box, outline=RED, width=2)
        card.paste(thumb, (27, 107))
    except Exception:
        pass
    ...
    qr_plate = Image.new("RGB", (qr_img.width + 16, qr_img.height + 16), INK)
    qr_plate.paste(qr_img, (8, 8))
    card.paste(qr_plate, (CARD_W - qr_plate.width - 30, 100))
    draw.text((CARD_W - qr_plate.width - 30, 100 + qr_plate.height + 8), "Scan to verify",
    ...
    for label, value in lines:
        ...
        draw.text((x, y + 21), str(value), font=font_value, fill=fill)
        y += 51

    # Chain of Custody badge, bottom-left of the text column
    badge_text = "\u26d3 CHAIN OF CUSTODY VERIFIED"
    draw.text((x, y + 4), badge_text, font=_load_font(14), fill=(0, 150, 90))

    # verdict pill, bottom left
    draw.rectangle([0, CARD_H - 46, CARD_W, CARD_H], fill=BLACK)
    draw.text((26, CARD_H - 32), "Scan the QR to re-verify this seal live against the SottoQR ledger.",
               font=_load_font(13), fill=MUTED)
